...
async function getWeatherData({ lat, lon }, { hourlyParam, dailyParam }) {
  try {
...
async function main() {
  ...
  const parameters = { hourlyParam, dailyParam };

  const coordinates = await getCoordinates(city);
  const weatherData = await getWeatherData(coordinates, parameters);
  console.log(weatherData);
  getHtml(weatherData)
}

async function getHtml(weatherData) {
  const htmlMarkup = `
      <div>
        <h1>Temprature </h1>
...
main();
const id = document.querySelector(".weather");
console.log(id);
